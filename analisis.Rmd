...
#ATENCION ADMINISTRATIVA CIVIL
```{r, echo = FALSE, warning = FALSE, out.width = "100%"}
#OBTENGO LOS PUNTOS DE ATENCION CIUDADANA BUSCANDO LAS DIRECCIONES QUE FIGURAN EN LA BASE DE DATOS:

#a_puntos = read.xlsx("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/ministerio-de-gobierno/puntos-atencion/puntos-de-atencion.xlsx") %>% 
...
#st_write(a_puntos, "data/a_puntos.geojson")

a_puntos = st_read("data/a_puntos.geojson") %>% 
  filter(!is.na(lat)) %>% 
  st_drop_geometry(geometry) %>% 
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
  conteo_interno("n_a_puntos")


a_vtv = read.xlsx("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/transporte-y-obras-publicas/verificacion-tecnica-vehicular-vtv/verificacion-tecnica-vehicular.xlsx") %>% 
  filter(!is.na(lat)) %>% 
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
  conteo_interno("n_a_vtv")
...
#buscar_osm("c_comercio", "shop")

#c_comercio_pt = c_comercio$osm_points %>% 
#  select(name)

#c_comercio_mpol = c_comercio$osm_multipolygons %>% 
#  select(name) %>% 
#  st_centroid() %>% 
#  st_transform(4326)

#c_comercio_pol = c_comercio$osm_polygons %>% 
#  select(name)%>% 
#  st_centroid() %>% 
#  st_transform(4326)

#c_comercio = rbind(c_comercio_pt,c_comercio_pol,c_comercio_mpol) %>% 
#  conteo_interno("n_c_comercio")

#st_write(c_comercio, "data/c_comercio.geojson")

c_comercio = st_read("data/c_comercio.geojson")

c_ferias = st_read("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/ministerio-de-espacio-publico-e-higiene-urbana/ferias-mercados/ferias-itinerantes-de-abastecimiento-barrial.geojson") %>% 
  conteo_interno("n_c_ferias")

#CONSTRUYO LA BASE DE FLORERIAS CON LAS DIRECCIONES:

#c_florerias = read.xlsx("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/atencion-ciudadana/puestos-de-flores/puestos-flores.xlsx") %>% 
#  mutate(direccion = paste0(Calle, " ", Altura, ", Ciudad de Buenos Aires, Argentina"))

#evito problemas por cortes en la busqueda y después guardo el archivo y desactivo esta parte del código porque se demora mucho
#Sys.sleep(1)

#c_florerias = c_florerias %>%
#  geocode(address = direccion, method = "osm")

#c_florerias = c_florerias %>% 
#  filter(!is.na(lat)) %>% 
#  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
#  conteo_interno("n_c_florerias")

#st_write(c_florerias, "data/c_florerias.geojson")

c_florerias = st_read("data/c_florerias.geojson") %>% 
  mutate(BARRIO = toupper(BARRIO))

c_eess = read.xlsx("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/ministerio-de-educacion/estaciones-servicio/estaciones_servicio_caba.xlsx") %>% 
  filter(!is.na(lat)) %>% 
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
  conteo_interno("n_c_eess")

c_cajeros = read.xlsx("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/secretaria-de-desarrollo-urbano/cajeros-automaticos/cajeros-automaticos.xlsx") %>% 
  filter(!is.na(lat)) %>% 
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
  select(id)

c_bancos = read.xlsx("https://cdn.buenosaires.gob.ar/datosabiertos/datasets/ministerio-de-economia-y-finanzas/bancos/bancos.xlsx") %>% 
  filter(!is.na(lat)) %>% 
  st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
  select(id)

c_bancosyc = rbind(c_bancos,c_cajeros) %>% 
  mutate(geos = as.character(geometry)) %>% 
  filter(!duplicated(geos)) %>% 
  conteo_interno("n_c_bancosyc")
```
...
bases_de_datos <- ls() %>%
  grep("^r_|^se_|^s_|^e_|^a_|^c_", ., value = TRUE) %>%
  lapply(get)
