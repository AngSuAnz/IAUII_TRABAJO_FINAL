...
barrios_datos_a = barrios_datos %>% 
  select(BARRIO, subindice) %>% 
  rename(si_a = subindice)
```
</details>
...
Para concluír, tomo todos los subindices y los pondero a su vez para poder obtener un valor agregado para cada barrio de la Ciudad Autónoma de Buenos Aires.

<details><summary> Código </summary> 
```{r unificado}
#LLAMO TODAS LAS BASES DE DATOS Y LAS FILTRO SEGUN SI SON DE LAS CATEGORIAS DE ANALISIS
bases_de_datos <- ls() %>%
  grep("^barrios_datos_", ., value = TRUE) %>%
  lapply(get)

# ARMO UNA BASE UNIFICADA HACIENDO LEFT_JOINS ITERATIVOS CON AS BD
indice_unificado <- bases_de_datos %>% 
  reduce(function(df1, df2) {
    left_join(df1, df2 %>% st_drop_geometry(), by = "BARRIO") %>%
      replace(is.na(.), 0)
...
barrios_datos = f_barrios_datos() %>% 
  mutate(indice = round((si_a*0.2 +
         ...
         popup = paste("<strong> BARRIO: </strong>", BARRIO,"<br> 
                 <strong> ÁREA: </strong>", AREAKM2, "km2<br> 
                 <strong> INDICE: </strong>",indice,"<br> 
                 <strong> -Autonomía: </strong>", si_a,"<br> 
                 <strong> -Cultura: </strong>", si_c,"<br> 
                 <strong> -Deporte: </strong>", si_d,"<br> 
                 <strong> -Educación: </strong>", si_e,"<br> 
                 <strong> -Medio Ambiente: </strong>", si_mb,"<br> 
                 <strong> -Recreación: </strong>", si_r,"<br> 
                 <strong> -Salud: </strong>", si_s,"<br> 
                 <strong> -Seguridad: </strong>", si_se,"<br> 
                 <strong> -Trámites: </strong>", si_t,"<br> 
                 <strong> -Transporte: </strong>", si_tp))

indice_unificado = barrios_datos
```
</details>
...
<details><summary> Código </summary> 
```{r}
mapa_indice = leaflet(st_zm(barrios) %>% st_transform(4326)) %>%
  addTiles() %>%
  addProviderTiles(providers$CartoDB.Positron, group = "OSM",
                   options = providerTileOptions(minzoom = 1, maxzoom = 15)) %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Satelite") %>%
  addLayersControl(
    baseGroups = c("OSM","Satelite"), 
    overlayGroups = c("Indice por barrio","Puntos Aleatorios"))%>% 
  
  addPolygons(data= indice_unificado,
              color = ~pal_i(indice),
              weight = 0.4,
              opacity = 0.7,
              fillColor = ~pal_i(indice),
              fillOpacity = 0.8,
              popup = ~popup,
              group = "Indice por barrio") %>% 
